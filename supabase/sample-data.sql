-- ============================================================
-- APadel — Sample Data  (safe to re-run multiple times)
-- Paste into Supabase SQL Editor → Run
-- ============================================================

-- ─── 0. Schema patches (safe to re-run) ──────────────────
-- join_code: lets Player 2 join an existing registration
alter table public.registrations add column if not exists join_code text;
create unique index if not exists idx_registrations_join_code on public.registrations(join_code) where join_code is not null;

-- ─── 1. Extensions ────────────────────────────────────────
create extension if not exists pgcrypto schema extensions;
set search_path to public, extensions;

-- ─── 1b. Admin account: ammaralkadyy@gmail.com ───────────
do $$
declare
  admin_id uuid := 'ffffffff-0000-0000-0000-000000000001';
begin
  -- Auth user
...
-- ─── 6. Auth users ────────────────────────────────────────
-- All three players / password: Padel2026!
insert into auth.users (
  id,
  instance_id,
  aud,
...
-- ─── 7. Profiles (DO NOT rely on trigger — insert manually) ──
...
-- ─── 10. Champions (for completed Spring Cup) ─────────────
insert into public.tournament_champions
  (tournament_id, position, player1_name, player2_name)
values
  ('b1000000-0000-0000-0000-000000000003', 1, 'Ahmed Hassan',  'Mohamed Ali'),
  ('b1000000-0000-0000-0000-000000000003', 2, 'Karim Sherif',  'Omar Farouk'),
  ('b1000000-0000-0000-0000-000000000003', 3, 'Youssef Nabil', 'Tarek Ibrahim')
